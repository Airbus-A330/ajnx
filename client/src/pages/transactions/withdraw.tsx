...
const Withdraw: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedAccount && Number(amount) > selectedAccount.availableBalance) {
    ...
    try {
      ...
      addToast({
        ...
        description: `$${Number(amount).toLocaleString()} has been withdrawn from your account`,
        color: 'success'
      });
      ...
      setFormError(err.response?.data?.message || 'Failed to process withdrawal');
    } finally {
      ...
      <Card className="bg-content1">
        <CardHeader className="flex gap-3">
          <div className="bg-danger/10 p-3 rounded-full">
            <Icon icon="lucide:minus" className="text-danger text-xl" />
          </div>
          <div className="flex flex-col">
            <h1 className="text-2xl font-bold">Withdraw Funds</h1>
            <p className="text-default-500">Withdraw money from your account</p>
          </div>
        </CardHeader>
          ...
          <CardBody className="gap-6">
            ...
            {selectedAccount && (
              <div className="bg-content2/30 p-3 rounded-lg">
                <p className="text-sm">
                  Available Balance: <span className="font-semibold">{selectedAccount.currency} {selectedAccount.availableBalance.toLocaleString()}</span>
                </p>
              </div>
            )}
            ...
            <div className="bg-content2/30 p-4 rounded-lg">
              <h3 className="text-md font-semibold mb-2">Withdrawal Information</h3>
              <ul className="space-y-2 text-sm">
                <li className="flex items-start gap-2">
                  <Icon icon="lucide:info" className="text-primary mt-0.5" />
                  <span>Funds will be deducted from your account immediately.</span>
                </li>
                <li className="flex items-start gap-2">
                  <Icon icon="lucide:info" className="text-primary mt-0.5" />
                  <span>Daily withdrawal limits may apply based on your account type.</span>
                </li>
                <li className="flex items-start gap-2">
                  <Icon icon="lucide:alert-triangle" className="text-warning mt-0.5" />
                  <span>For security reasons, large withdrawals may require additional verification.</span>
                </li>
              </ul>
            </div>
          </CardBody>
